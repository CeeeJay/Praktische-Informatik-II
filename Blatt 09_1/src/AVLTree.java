...
public class AVLTree {
	...
	public void insertNode(int value) {
		/*
		**	Hier wird kontrolliert, ob der Baum bisher leer ist.
		**	Sollte dies der Fall sein wird einfach ein neues Element erzeugt.
		**	Danach wird die Methode verlassen.
		*/
		if(this.isEmpty()){
		...
		while((currBal=getBalance(currentNode)) > 1||currBal < -1){
			...
			if(currBal < -1){
				if(currLeftBal <= 1&&currLeftBal >=-1){
					currentNode = doRightRotation(currentNode);
					if(currentNode.parent==null)
						root = currentNode;
					else if(left)
						lastParent.left = currentNode;
					else
						lastParent.right = currentNode;
					if(currentNode.parent != null){
						currentNode = currentNode.parent;
						lastParent = currentNode.parent;
					}else{
						currentNode = root;
						lastParent = null;	
					}
				...
				if(currRightBal <= 1&&currRightBal >=-1){
					currentNode = doLeftRotation(currentNode);
					if(currentNode.parent==null)
						root = currentNode;
					else if(left)
						lastParent.left = currentNode;
	...
	//Noch Muell? ODer etwa nicht?
	public void deleteNode(int value) {
		AVLNode currentNode = root;
		AVLNode lastParent = null;
		
		while(currentNode.data != value) {
			if(currentNode.left != null && lastParent!=currentNode) {
				lastParent=currentNode;
				currentNode=currentNode.left;
			} else {
				if(currentNode.right!=null) { lastParent=currentNode; currentNode=currentNode.right; }
				else
					if(currentNode.parent==null || (currentNode.left==null
						&& currentNode.right == null
						&& currentNode==lastParent.right)) { System.out.println("Es ist kein Element mit dem gesuchten Wert vorhanden!"); return; }
					else { currentNode = lastParent; }
			}
		}
		
		if (currentNode == root) {
			 if(currentNode.left!=null) root =null;
		} else {
			if(currentNode==lastParent.left) lastParent.left = null;
