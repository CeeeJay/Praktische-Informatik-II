...
public class StackRechner_Falsch {
	
	private String infixToPostfix(String infix){
		// String, der am Ende zurückgegeben wird
		String returnString = "";
		// Ein Array von boolschen Variablen, die angeben, ob die Operation am Ende seiner Schleife hinten angefügt wird (0='+', 1='-', 2='*', 3='/')
		boolean[] operationVorhanden = new boolean[4];
		...
		while(add.hasMoreTokens()){
			...
			while(sub.hasMoreTokens()){
				...
				while(mul.hasMoreTokens()){
					...
					// Siehe '+'
					if(operationVorhanden[3])
						returnString += " /";
				}
				// Siehe '+'
				if(operationVorhanden[2])
					returnString += " *";
			}
			// Siehe '+'
			if(operationVorhanden[1])
				returnString += " -";
		}
		// War der entsprechende boolsche Wert auf "true" wird das '+' hinten angehängt
		if(operationVorhanden[0])
			returnString += " +";
//		System.out.println(returnString); // Ehemalige Testausgabe
	...
	private int calculateValue(String postfix){
		...
		while(st.hasMoreTokens()){
			...
			// Sollte das misslingen (im Fall, dass es sich um eine Operation handelt oder zuvor "übersehene" Fehler auftreten)
